...
static int32_t subghz_file_encoder_worker_thread(void* context) {
    ...
    /* A broken/missing CC1101 completion indication must not leave RAW TX
     * running forever.  Normal hardware exits immediately; the timeout is a
     * safety net for the ESP32 port. */
    uint32_t tx_wait_ticks = 0;
    while(instance->device && !subghz_devices_is_async_complete_tx(instance->device) &&
          instance->worker_running && tx_wait_ticks++ < 600) {
        furi_delay_ms(5);
    }

    FURI_LOG_I(TAG, "End transmission");
    while(instance->worker_running) {
        if(instance->worker_stopping && !instance->callback_sent) {
...
SubGhzFileEncoderWorker* subghz_file_encoder_worker_alloc(void) {
    ...
    /* RAW TX is intentionally fed incrementally from SD.  A smaller queue
     * avoids reserving 8 KB while the SubGHz UI is already near its limit. */
    instance->stream = furi_stream_buffer_alloc(sizeof(int32_t) * 512, sizeof(int32_t));
