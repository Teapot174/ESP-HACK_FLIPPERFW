...
SubGhzWorker* subghz_worker_alloc(void) {
    ...
    /* RAW recording writes through FlipperFormat/FATFS from this worker's
     * decoder callback.  That call chain needs more stack than normal protocol
     * decoding on ESP32. */
    instance->thread =
        furi_thread_alloc_ex("SubGhzWorker", 4096, subghz_worker_thread_callback, instance);

    /* Keep enough burst history for normal decoding while leaving DRAM
     * for the SubGHz UI and RAW recorder on the WROOM. */
    instance->stream =
