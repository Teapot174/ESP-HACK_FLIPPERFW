...
| **DIY ESP32-WROOM + SH1106** | ESP32 (Xtensa LX6) | SH1106 128×64 (I2C) | 4× buttons | CC1101 | — | RMT TX + RX | SPI |
...
python winbuild.py build --board wroom
...
ESP32-WROOM + SH1106 wiring for the `wroom` target:

| Function | GPIO |
|---|---:|
| OLED SDA / SCL (0x3C) | 21 / 22 |
| Up / Down / OK / Back | 27 / 26 / 33 / 32 |
| SD CS / MOSI / SCLK / MISO | 15 / 13 / 14 / 17 |
| CC1101 CS / SCK / MOSI / MISO / GDO0 | 5 / 18 / 23 / 19 / 4 |
| IR TX / RX | 16 / 35 |

Linux/macOS build: `./build.sh --board wroom --build-only`. The target assumes
a common 4 MB ESP32-WROOM module. GPIO15 is a boot-strapping pin, so the SD
module must not pull CS low while the ESP32 resets.

Boards: `t_embed` (default), `esp32s3`, `waveshare_c6`, `wroom`. Override defaults with `ESP_IDF_DIR` and `ESPPORT` env vars. `monitor --reset` only works on USB-UART bridges, not on the ESP32-S3 native USB-Serial/JTAG — use `flash` or `all` to capture boot logs.
...
- **Display** sends the original 128×64 mono framebuffer directly to SH1106, or aspect-scales it to RGB565 on color LCD targets
