...
# Keep service stacks conservative: they stay alive for the whole firmware and
# commonly enter SPI/FATFS from callbacks.  Short-lived application threads on
# the 4 MB WROOM have to use a smaller floor or the collection of 1 KB apps
# consumes most of internal DRAM before the selected app even starts.
ESP32_MIN_STACK_SIZE = 2048
ESP32_SERVICE_MIN_STACK_SIZE = 4096


def render_internal_entry(app: FlipperApplication) -> str:
    minimum = ESP32_SERVICE_MIN_STACK_SIZE if app.apptype == FlipperAppType.SERVICE else ESP32_MIN_STACK_SIZE
    stack = max(app.stack_size, minimum)
    return (
        "    {"
