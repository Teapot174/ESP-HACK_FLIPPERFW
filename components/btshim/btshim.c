...
static bool bt_stack_is_initialized(void) {
...
static void bt_apply_settings(Bt* bt) {
    ...
    /* Settings can be edited while Loader has suspended the controller to make
     * room for an application.  Persist the desired state now; Loader will
     * restart the stack after the application exits. */
    if(!bt_stack_is_initialized()) {
        FURI_LOG_I(TAG, "BLE stack is suspended, deferring radio setting");
        return;
    }
    if(bt->bt_settings.enabled) {
        furi_hal_bt_start_advertising();
...
static void bt_handle_stop_stack(Bt* bt) {
    ...
    if(bt->status == BtStatusOff) {
        /* BtStatusOff also means "initialized but not advertising".  Do not
         * return in that state: the controller still owns its large heap. */
        FURI_LOG_I(TAG, "BLE radio idle but stack is still initialized");
    }
...
static void bt_handle_start_stack(Bt* bt) {
    if(!bt->bt_settings.enabled) {
        FURI_LOG_I(TAG, "BLE disabled in settings, keeping stack stopped");
        return;
    }

    if(bt_stack_is_initialized()) {
        FURI_LOG_I(TAG, "BLE stack already started");
        return;
    }
...
int32_t bt_srv(void* p) {
    ...
    while(1) {
        ...
        if(message.type == BtMessageTypeUpdateStatus) {
            ...
            bool was_initialized = bt_stack_is_initialized();
            bt_handle_stop_stack(bt);
            if(message.result) *message.result = was_initialized;
