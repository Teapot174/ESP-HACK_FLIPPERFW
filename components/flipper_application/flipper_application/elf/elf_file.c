...
/* Convert the writable data-bus alias used while relocating a FAP to the
 * instruction-bus alias used for function pointers and entry points. */
#if defined(CONFIG_IDF_TARGET_ESP32)
static inline uint32_t elf_data_to_instruction(uint32_t address) {
    const void* ptr = (const void*)address;
    return esp_ptr_in_diram_dram(ptr) ? (uint32_t)esp_ptr_diram_dram_to_iram(ptr) : address;
}
#define PSRAM_DATA_TO_INST(addr) elf_data_to_instruction((uint32_t)(addr))
...
static ELFLoadSectionResult
    ...
    if(section_header->sh_flags & SHF_EXECINSTR) {
        /* Classic ESP32 cannot execute from ordinary DRAM. Allocate from the
         * dual-mapped D/IRAM heap, retain the IRAM allocation for free(), and
         * perform file I/O/relocations through its byte-addressable DRAM alias. */
        size_t alignment = section_header->sh_addralign;
        if(alignment < sizeof(uint32_t)) alignment = sizeof(uint32_t);
        section->allocation = heap_caps_aligned_alloc(
            alignment, section_header->sh_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_EXEC);
        if(section->allocation && esp_ptr_in_diram_iram(section->allocation)) {
            section->data = esp_ptr_diram_iram_to_dram(section->allocation);
        } else {
            if(section->allocation) heap_caps_free(section->allocation);
            section->allocation = NULL;
            section->data = NULL;
        }
    } else {
        section->allocation = heap_caps_aligned_alloc(
        ...
        section->data = section->allocation;
    }
#elif defined(ESP_PLATFORM)
    ...
    section->allocation = section->data;
#else
    section->data = aligned_malloc(section_header->sh_size, section_header->sh_addralign);
    section->allocation = section->data;
#endif
    if(!section->data) {
        FURI_LOG_E(
            TAG,
            "Cannot allocate %lu bytes for ELF section (flags 0x%lX)",
            (unsigned long)section_header->sh_size,
            (unsigned long)section_header->sh_flags);
...
#ifdef ESP_PLATFORM
            if(itref->value.allocation) heap_caps_free(itref->value.allocation);
#else
            aligned_free(itref->value.allocation);
#endif
            free((void*)itref->key);
        }
