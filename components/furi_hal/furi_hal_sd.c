...
#define SD_MAX_FREQ (10 * 1000) /* Safer for jumper-wired WROOM boards */
#else
#define SD_MAX_FREQ (20 * 1000) /* 20 MHz for integrated/shared-bus boards */
#endif
#define SD_BOUNCE_SECTORS 8 /* 4 KiB persistent DMA bounce buffer */

#ifndef BOARD_PIN_SD_MOSI
#define BOARD_PIN_SD_MOSI BOARD_PIN_LCD_MOSI
#endif
#ifndef BOARD_PIN_SD_SCLK
...
#if defined(CONFIG_IDF_TARGET_ESP32)
/* Some otherwise functional SPI SD cards reject CMD59 (CRC_ON_OFF). ESP-IDF
 * 5.4 treats that response as fatal. Keep CRC enabled whenever the card
 * supports it, but allow these cards to continue at the conservative WROOM
 * bus frequency. The linker wraps only the call made by ESP-IDF's SD init. */
extern esp_err_t __real_sdmmc_init_spi_crc(sdmmc_card_t* card);

esp_err_t __wrap_sdmmc_init_spi_crc(sdmmc_card_t* card) {
    const esp_err_t err = __real_sdmmc_init_spi_crc(card);
    if(err == ESP_ERR_NOT_SUPPORTED) {
        ESP_LOGW(TAG, "SD card rejected CMD59; continuing without SPI CRC");
        return ESP_OK;
    }
    return err;
}
...
static bool sd_prepare_card(void) {
    ...
    do {
        const spi_bus_config_t bus_config = {
            ...
            .quadwp_io_num = -1,
            .quadhd_io_num = -1,
            .max_transfer_sz = 512 * SD_BOUNCE_SECTORS,
        };
        ret = spi_bus_initialize(SD_SPI_HOST, &bus_config, SPI_DMA_CH_AUTO);
        if(ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) break;
